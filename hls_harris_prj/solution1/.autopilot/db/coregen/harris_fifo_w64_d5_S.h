...
// ==============================================================
#ifndef harris_fifo_w64_d5_S_HH_
#define harris_fifo_w64_d5_S_HH_

#include <systemc>
using namespace std;
SC_MODULE(harris_fifo_w64_d5_S) {
    static const unsigned int DATA_WIDTH = 64;
    static const unsigned int ADDR_WIDTH = 3;
    static const unsigned int harris_fifo_w64_d5_S_depth = 6;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[harris_fifo_w64_d5_S_depth];
    ...
    SC_CTOR(harris_fifo_w64_d5_S) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < harris_fifo_w64_d5_S_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (harris_fifo_w64_d5_S_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < harris_fifo_w64_d5_S_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (harris_fifo_w64_d5_S_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > harris_fifo_w64_d5_S_depth) {
...
#endif //harris_fifo_w64_d5_S_HH_
